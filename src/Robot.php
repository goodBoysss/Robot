...
//        }
    ...
    private function getRobotUrl($option = array())
    {
        ...
        } else if (!empty(getenv("ROBOT_URL"))) {
            $robotUrl = getenv("ROBOT_URL");
        } else if (!empty(getenv("QYWX_ROBOT_URL"))) {
            $robotUrl = getenv("QYWX_ROBOT_URL");
        } else if (!empty(getenv("DD_ROBOT_URL"))) {
            $robotUrl = getenv("DD_ROBOT_URL");
        } else if (!empty(getenv("FS_ROBOT_URL"))) {
            $robotUrl = getenv("FS_ROBOT_URL");
        }

        if (empty($robotUrl)) {
            throw new \Exception('机器人消息发送失败，机器人链接不能为空', 990020);
        }
    ...
    private function getRobotType($robotUrl)
    {
        ...
        if (empty($this->ROBOT_TYPES[$robotType])) {
            throw new \Exception('机器人地址不合法或暂不支持该发送通道', 990020);
        }
     ...
     * @return array
     */
    public function sendMarkdownMsg($content, $option = array())
    {
        try {
            //获取机器人url
            $robotUrl = $this->getRobotUrl($option);
